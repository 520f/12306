        ...
        while 1:
            try:
                ...
                if queryResult.get("status"):
                    ...
                    print(f"当前时间为: {now.strftime('%Y-%m-%d %H:%M:%S.%f')}:正在第{num}次查询 停留时间：{random_time} 乘车日期: {','.join(TickerConfig.STATION_DATES)} 车次：{','.join(TickerConfig.STATION_TRAINS) or '所有车次'} 下单无票{nateMsg} 耗时：{(datetime.datetime.now() - now).microseconds / 1000}秒,请求CDN地址: {queryResult.get('cdn')}")
